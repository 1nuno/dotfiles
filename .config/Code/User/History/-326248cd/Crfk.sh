...
#phpmyadmin
docker run -dit --name phpmyadmin --network global-network -e PMA_HOST=mariadb -p 8080:80 phpmyadmin


#rabbitmq
docker run -d --hostname my-rabbit --name rabbitmq --network global-network -p 8080:15672 rabbitmq:3-management
