...
def get_filtered_clients():
    try:
        ...
        if 'item_bought' in request_body:
            filter_by_item = sql.SQL('WHERE {} = ANY(agregated_items.last_items_bought)').format(sql.Literal(request_body['item_bought']))
        else:
            filter_by_item = sql.SQL('')

        if 'last_purchase_date' in request_body:
            filter_by_item = sql.SQL('WHERE {} = ANY(agregated_items.last_items_bought)').format(sql.Literal(request_body['item_bought']))
            filter_by_date = sql.SQL('WHERE DATE(timestamp_column) = {}').format(sql.Literal(request_body['last_purchase_date']))
