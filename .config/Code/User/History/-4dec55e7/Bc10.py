...
def update_item(id):
    try:
        ...
        cols = [sql.Identifier(i) for i in request_body.keys()]
        values = [sql.Literal(i) for i in request_body.values()]
        cols_values = [item for sublist in zip(cols,values) for item in sublist]
        placeholders = ','.join(['{}={}'] * len(cols))
        query = sql.SQL(''' DO $$
                    BEGIN
                        UPDATE item SET %s WHERE id={};
                        IF not found then RAISE EXCEPTION 'Item not found';
                        END IF;
                    END $$;''' % placeholders).format(*cols_values,sql.SQL(str(id)))
        cur.execute(query)
        conn.commit()
        response = {'status': StatusCodes['success'],
...
@app.route('/proj/api/cart/<int:item_id>', methods=['DELETE'])
...
def get_filtered_clients():
    try:
        ...
        request_body = flask.request.get_json()
        field_types = {"item":str}
        is_broken_body = verify_request_body(request_body,field_types)
        if is_broken_body is not None:
            return is_broken_body

        if 'item' in request_body:
            filter_by = sql.SQL('WHERE {} = ANY(agregated_items.last_items_bought)').format(sql.Literal(request_body['item']))
        else:
            filter_by = sql.SQL('')
        ...
        query = sql.SQL('''WITH last_purchase_clients AS (
                                                    ...
                                                    FROM purchase
                                                    JOIN client ON client.id = purchase.client_id
                    ...
                    JOIN agregated_items ON agregated_items.purchase_id = last_purchase_clients.id
                    {};''').format(filter_by)
        cur.execute(query)
        conn.commit()
        values = cur.fetchall()
