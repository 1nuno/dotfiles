...
class TjscSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in links:
            yield scrapy.Request(url=link, headers=self.headers, callback=self.parse_data, meta={'page': response.meta.get('page')}, errback=self.onerror,
                dont_filter=True)
        ...
        for i in range(2,total_page_num+1):
            ...
            yield scrapy.Request(url='https://www.uc.pt/dsjf', headers=self.headers, callback=self.parse, meta={'page': i}, method="POST", body=self.payload, errback=self.onerror, dont_filter=True)
