...
#rabbitmq
docker build -t rabbitmq:mvp5 ./rabbitmq
docker run -d --hostname my-rabbit --name rabbitmq --network rabbitmq-network rabbitmq:mvp5


#sub
docker build -t sub:mvp5 ./sub
docker run -dit --name sub --network rabbitmq-network sub:mvp5 
docker network connect mariadb-network sub


#pub
docker build -t pub:mvp5 ./pub
docker run -dit --name pub --network rabbitmq-network -e LENGTHTOKEN=5 -e NUMTOKENS=3 -e FREQUENCY=0.5 pub:mvp5 
