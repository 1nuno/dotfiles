...
#rabbitmq
docker build -t rabbitmq:mvp3 ./rabbitmq
docker run -d --hostname my-rabbit --name rabbitmq --network rabbitmq-network rabbitmq:mvp3


#sub
docker build -t sub:mvp3 ./sub
docker run -dit --name sub --network rabbitmq-network sub:mvp3 
docker network connect mariadb-network sub


#pub
docker build -t pub:mvp3 ./pub
docker run -dit --name pub --network rabbitmq-network pub:mvp3 
