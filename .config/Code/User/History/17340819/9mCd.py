...
def get_item_list():
    try:
        ...
        limit = request_body.get("limit","count(*) OVER ()")
        category = request_body.get("category","cat.name")
        sort = request_body.get("sort","ctid")
        cur.execute('SELECT name FROM category;')
        categories = list(list(zip(*cur.fetchall()))[0]) + ["cat.name"]
        warning = ''
        if sort not in ("price","name","ctid"):
            sort = "ctid"
            warning += " Warning: The sort parameter you provided is not valid. sort was discarted."
        if category not in categories:
            category = "cat.name"
            warning += " Warning: The category parameter you provided is not valid. category was discarted."
        ...
        query = sql.SQL(''' WITH items_sum AS (
                         ...
                         final_output AS (
                                            SELECT item.id,
                                                   ...
                                                   (ROW_NUMBER() OVER (ORDER BY "item".{}) - 1) / %s + 1 AS page
                                            FROM item
                    ...
                    SELECT * 
                    FROM final_output
                    WHERE page = {};''').format(sql.Identifier(sort),limit,category,page)
        cur.execute(query)
        conn.commit()
        values = cur.fetchall()
        data = []
        ...
        response = {'status': StatusCodes['success'],
                    'message': f"Items retrieved successfully :).{warning}",
                    'data': values}
