...
def create_tables():
    try:
        ...
        query  = '''DROP TABLE IF EXISTS item;
                    ...
                    ALTER TABLE item ADD CONSTRAINT nonnegative-stock CHECK (stock >= 0);
                    ALTER TABLE item ADD CONSTRAINT positive-price CHECK (price > 0);
                    ALTER TABLE purchase_item ADD CONSTRAINT nonnegative-quantity CHECK (quantity >= 0);
                    ALTER TABLE cart_item ADD CONSTRAINT nonnegative-quantity CHECK (quantity >= 0);'''
        cur.execute(query)
        conn.commit()
        print("Success")
